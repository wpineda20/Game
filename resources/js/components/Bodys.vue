<template>
  <main role="main" class="container-fluid" id="app">
    <div class="row">
      <div class="col-12">
        <div class="card-body-title text-center mx-auto">
          <h1 class="text-center">Memory Game: Body</h1>
          <p>
            The objective is to memorize the location of the different cards in
            order to successively turn over the 2 identical cards that form a
            pair, in order to take them away. The game is over when all the
            pairs are found.
          </p>
        </div>
        <div class="text-center mt-3">
          <ul
            class="mx-auto text-center justify-content-center"
            style="list-style: none; display: flex"
          >
            <li>
              <a href="/memoryAnimals" class="btn btn-nav-register">Level 1</a>
            </li>
            <li>
              <a href="/memoryColors" class="btn btn-nav-register">Level 2</a>
            </li>
            <li>
              <a href="/memoryBody" class="btn btn-nav-register">Level 3</a>
            </li>
          </ul>
        </div>
        <p class="mt-5">
          <span class="h5">Attempts: </span>
          {{ intentos }}/{{ MAXIMOS_INTENTOS }}&nbsp;<span class="h5"
            >Success:
          </span>
          {{ aciertos }}
        </p>
    ...
    <div class="row justify-content-center mt-3">
      <audio class="audio" controls autoplay loop>
        <source src="media/Vocabulary.mp3" type="audio/mpeg" />
...
<script>
let MAXIMOS_INTENTOS = 8, // Intentos máximos que tiene el jugador
  COLUMNAS = 4, // Columnas del memorama
  SEGUNDOS_ESPERA_VOLTEAR_IMAGEN = 1, // Por cuántos segundos mostrar ambas imágenes
  NOMBRE_IMAGEN_OCULTA = "./img/question3.png"; // La imagen que se muestra cuando la real está oculta
export default {
  data: () => ({
  ...
  methods: {
    ...
    indicarFracaso() {
      Swal.fire({
        title: "You Lost",
        html: `
                <img class="img-fluid" src="./img/gameover.png" alt="Perdiste">
                <p class="h4">Exhausted your Attempts</p>`,
